...
| :--- | :--- | :---: | :--- |
| **2.5.1** | Первый запуск, онбординг, 3 типа решений | 🔄 **Частично (75%)** | Гостевой режим работает (нет ввода e-mail/телефона). Предустановлен стартовый баланс, подсказка в облачке Финни. **Не хватает:** отдельного интерактивного пошагового визарда-слайдера с картинками правил. |
...
| **2.5.2** | Настройка внешности (не менее 9 комбинаций) | ⚠️ **Адаптировано (100%)** | Реализована кастомизация 5 интерактивных подиумов (Изумруд, Звёзды, Цветы, Космос, Облако) + имя + 3 стадии эволюции с разными анимациями ($5 \times 3 = 15$ комбинаций). Наложение статичных курток/шапок поверх цельного GIF удалено по просьбе пользователя из-за неестественности наложения. |
| **2.5.3** | Главный экран: питомец, баланс, цель, квесты | ✅ **Реализовано (100%)** | Единый дашборд: Финни слева с облачком реплик, показатели сытости/настроения, карточки баланса и копилки справа, прогресс цели, образовательное задание, кнопки быстрых действий. |
...
| **2.5.5** | Планирование бюджета: 3 направления, план vs факт | 🔄 **Реализовано (90%)** | Правило 3 конвертов (обязательные, желания, сбережения) с шагом 10 монет, блокировкой перерасхода и текстово-числовым отчетом «План vs Факт». **Не хватает:** визуальной круговой диаграммы (PieChart). |
| **2.5.6** | Покупки: обязательные/желаемые, блок минуса | 🔄 **Реализовано (90%)** | 8 товаров в 2 категориях, блокировка покупки при нехватке монет с объяснением, пополнение сытости/настроения, реплики благодарности. **Не хватает:** анимаций процесса поедания еды/игры с мячом. |
| **2.5.7** | Цели: расчет срока, пополнение, снятие | 🔄 **Реализовано (85%)** | Предустановленные цели + добавление своей, расчет остатка периодов $T = \lceil (Target - Savings) / Avg \rceil$, пополнение (+20, +50, +100), снятие с предупреждением, двусторонний перевод. **Не хватает:** начисления банковского процента на остаток. |
| **2.5.8** | Задания: 3 темы, ситуации с выбором | 🔄 **Реализовано (95%)** | 6 практических кейсов по ГОСТ/стандартам для 2 возрастов (7–8 и 9–11 лет). Разбор ошибок, мудрость Финни, двухколоночная модалка с автопроигрыванием 1 такта радости или грусти. **Не хватает:** таймера и динамической загрузки из облака. |
...
| **2.5.10** | 3 состояния и 3 стадии развития питомца | ✅ **Реализовано (100%)** | 4 эмоции (Idle, Wave, Proud, Sad) и 3 стадии роста (Малыш, Юниор, Мастер с золотой аурой). 12 рисованных GIF-анимаций. Автопроигрывание ровно 1 такта при любых действиях. |
| **2.5.11** | История периодов и словарь терминов | 🔄 **Реализовано (80%)** | Финансовый словарь на 8 терминов с примерами из жизни. История периодов сохраняется в `PeriodSummary`. **Не хватает:** отдельного графического экрана с динамикой трат по всем периодам. |
| **2.5.12** | Кабинет родителя с защитным барьером | 🔄 **Реализовано (85%)** | Арифметический барьер (например, $7 \times 8 = 56$), статистика решенных задач, начисление карманных денег (+100 монет), ручная смена стадии роста, сброс профиля. **Не хватает:** настраиваемого PIN-кода и email-отчетов. |
| **2.5.13** | Демо-режим и сброс тестового профиля | ✅ **Реализовано (100%)** | Управляется тумблером в Кабинете родителя (по умолчанию выключен). Позволяет экспертам за пару минут пройти периоды 1..5+ и оценить эволюцию персонажа. |
...
| **3.3** | Готовность к RuStore, подписанный APK | ✅ **Реализовано (100%)** | Подписанный релизный APK (81.5 МБ), карточка `RUSTORE_CARD.md`, иконка 512×512, скрипт `build_apk.ps1`. |
| **3.6** | Доступность: кнопки $\ge 48$dp, шрифт $\ge 16$sp | ✅ **Реализовано (100%)** | Все кнопки имеют сенсорную область не менее $48 \times 48$ dp. Шрифт Montserrat. Полный отказ от эмодзи в пользу презентационных векторных иконок. |

---

## 4. Детальный разбор: чего не хватает до идеала ТЗ (Бэклог развития)

Для честного и прозрачного позиционирования проекта на конкурсе зафиксирован список возможностей, рекомендуемых к реализации в следующих версиях:

1. **Интерактивный слайдер онбординга (Onboarding Wizard):**
   - *Текущее состояние:* Приложение стартует сразу на главном экране с балансом 400 монет и приветственной репликой в облачке Финни.
   - *Что нужно добавить:* 3–4 полноэкранных вводных слайда при первом запуске (знакомство с Финни, правило 3 конвертов, как копить на мечту), с кнопкой «Начать приключение».
2. **Визуализация структуры бюджета (Круговая диаграмма):**
   - *Текущее состояние:* В модалке «План vs Факт» отображается форматированный текстово-числовой отчет с балансами и бейджами.
   - *Что нужно добавить:* Графический компонент `PieChart` или `DonutChart` (на базе `Microcharts` или `Microsoft.Maui.Graphics`) для цветной визуализации соотношения обязательных расходов, желаний и копилки.
3. **Механика начисления банковских процентов на копилку:**
   - *Текущее состояние:* В словаре и заданиях объясняется суть сложного процента, но на остаток копилки между периодами проценты не начисляются автоматически.
   - *Что нужно добавить:* Автоматическое начисление $+3..5\%$ годовых (в пересчете на период) на средства в копилке при завершении периода с выводом реплики Финни «Твои монетки поработали и принесли доход!».
4. **Специфические анимации действий в магазине:**
   - *Текущее состояние:* При покупке любого товара Финни радуется общей анимацией `proud.gif`.
   - *Что нужно добавить:* Индивидуальные микро-анимации для каждого предмета (котик ест рыбку из миски, гоняет лазером точку, надевает ошейник).
5. **Экран исторической аналитики (График динамики периодов):**
   - *Текущее состояние:* В профиле сохраняются данные `PeriodSummary` по каждому периоду.
   - *Что нужно добавить:* Отдельный экран «История успехов» с графиком накоплений и структуры расходов за последние 5–10 периодов.
6. **Настраиваемый PIN-код родителя и email-уведомления:**
   - *Текущее состояние:* Защита кабинета реализована через генерацию случайного арифметического примера (например, $7 \times 8$).
   - *Что нужно добавить:* Возможность установить постоянный 4-значный цифровой PIN-код и настроить отправку еженедельного отчета на email родителя.
7. **Облачная синхронизация заданий:**
   - *Текущее состояние:* Все задания хранятся локально в `ContentRepository.cs`.
   - *Что нужно добавить:* Фоновое обновление базы заданий и финансового календаря через REST API Департамента финансов г. Москвы.
...
| **14** | Сброс тестового профиля | Вход в Кабинет родителя $\rightarrow$ нажатие «СБРОСИТЬ тестовый профиль» $\rightarrow$ профиль мгновенно возвращается к первоначальному эталонному состоянию. |
...
Проект снабжен комплектом из **15 автоматизированных xUnit тестов** (`FinAPP.Tests`), покрывающих ключевую бизнес-логику и низкоуровневую обработку анимаций:
...
- `TransferBetweenWalletAndSavings_ShouldUpdateBothBalances` — проверка двустороннего перевода между кошельком и копилкой.
...
- `PetProfile_ResetToDefault_RestoresInitialState` — проверка чистого сброса тестового профиля для экспертов.

Запуск тестов:
```powershell
dotnet test FinAPP.Tests/FinAPP.Tests.csproj
```
Результаты: **15 passed, 0 failed, 0 skipped, duration: 84 ms**.
...
```
Файл: `dist/FinAPP.apk` (81.54 МБ, SHA-256: `0920888517814ED0C64EBDDB8E34982D15023B81F68734748CF9638825B287C9`).
